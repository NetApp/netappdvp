...
func EmsInitialized(driverName string, api *ontap.Driver) {
	...
	_, emsErr := api.EmsAutosupportLog(strconv.Itoa(CurrentDriverVersion), false, "initialized", myHostname, driverName+" docker volume plugin initialized", 1, "netappdvp", 6)
	if emsErr != nil {
