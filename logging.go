...
func initLogging() error {
	...
	logFileHook, err := NewFileHook(*driverID)
	if err != nil {
...
func NewFileHook(logName string) (*FileHook, error) {
	...
	// If LOG_ROOT doesn't exist, make it
	dir, err := os.Lstat(LOG_ROOT)
	if os.IsNotExist(err) {
		if err := os.MkdirAll(LOG_ROOT, 0755); err != nil {
			return nil, fmt.Errorf("Could not create log directory %v. %v", LOG_ROOT, err)
		}
	}
	// If LOG_ROOT isn't a directory, return an error
	if dir != nil && !dir.IsDir() {
		return nil, fmt.Errorf("Log path %v exists and is not a directory, please remove.", LOG_ROOT)
	}
	...
	switch runtime.GOOS {
	case utils.Linux:
		logFileLocation = LOG_ROOT + "/" + logName + ".log"
		break
	case utils.Darwin:
		logFileLocation = LOG_ROOT + "/" + logName + ".log"
		break
